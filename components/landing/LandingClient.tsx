...
import { Users, Zap, Github, LogOut, UserCircle, Timer, Bell, BarChart2, Link2 } from 'lucide-react'
...
const features = [
  ...
  },
  {
    ...
    desc: 'Copy the link or share to WhatsApp, Telegram, Discord, Slack and more.',
  },
  {
    icon: Timer,
...
function LandingContent({ user, profileUsername }: LandingClientProps) {
  ...
  // Show welcome toast after OAuth redirect
  useEffect(() => {
    ...
    <div className="flex flex-col min-h-screen" style={{ background: 'var(--bg-primary)' }}>
          ...
          {user ? (
                ...
                >
                  ...
                  <div className="py-1">
                    <button
                      onClick={() => {
                        setShowUserMenu(false)
                        router.push(`/profile/${profileUsername ?? user.id}`)
                      }}
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer disabled:opacity-50"
                style={{
      ...
      <section className="relative flex-1 flex flex-col items-center justify-center px-5 sm:px-8 py-16 sm:py-24 text-center overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto">
          ...
          {/* CTAs */}
          <div
            className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-4 animate-fade-up"
            style={{ animationDelay: '240ms' }}
          >
            <button
              onClick={handleCreateSession}
              disabled={isCreating}
              className="w-full sm:w-auto min-w-[180px] px-6 py-3 rounded-xl font-medium text-sm transition-all duration-150 cursor-pointer disabled:opacity-70"
              style={{
            ...
            <div className="flex w-full sm:w-auto gap-2">
              <input
                ...
                className={cn(
                  'flex-1 sm:w-64 px-4 py-3 rounded-xl text-sm outline-none transition-all duration-200',
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-150 cursor-pointer disabled:opacity-50"
                style={{
                  background: 'var(--bg-elevated)',
                  border: '1px solid var(--border)',
                  color: 'var(--text-primary)',
          ...
          {!user && (
            <div
              className="mt-6 p-4 rounded-2xl max-w-xs mx-auto animate-fade-up"
              style={{
                background: 'var(--bg-secondary)',
                border: '1px solid var(--border)',
            ...
            >
              <p className="text-sm mb-3" style={{ color: 'var(--text-muted)' }}>
                Sign in to track your focus stats
              </p>
      ...
      <section className="px-5 sm:px-8 py-16 sm:py-24 max-w-5xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {features.map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className="p-5 rounded-2xl animate-fade-up group transition-all duration-200"
              style={{
                background: 'var(--bg-elevated)',
                border: '1px solid var(--border)',
                boxShadow: 'var(--shadow-sm)',
      ...
      {/* ── How it works ── */}
      <section
        className="px-5 sm:px-8 py-16 sm:py-24 w-full"
